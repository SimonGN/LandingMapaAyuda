...
const Opportunity = ({ country, photo, t, theme }) => {
	const displayCardCountry = () => {
		return cardCountry.map((card, i) => {
			const { country, photo } = card;
			return <CardCountry key={i} photo={photo} country={country} />;
		});
	};
	const optionCountry = () => {
		...
		<OpportunityStyle photo={photo} country={country}>
			<div className='textAll'>
				...
				<div className='filter'>
					<ParagraphBodySmall size='16px' color={theme.colors.dark40}>
						{t('desplegable')}
					</ParagraphBodySmall>
					<div className='dropDown'>
						<Dropdown options={optionCountry()} placeholder={t('desplegable2')} />{' '}
						<img src='/static/svg/iconDropDown.svg' />
					</div>
				</div>
			</div>
			<div className='card'>{displayCardCountry()}</div>
